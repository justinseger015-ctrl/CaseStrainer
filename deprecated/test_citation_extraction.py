...
def test_extract_citations():
    ...
    for test_case in test_cases:
        ...
        try:
            ...
            result, debug_info = extract_citations(test_case['input'], return_debug=True)
            ...
            for i, citation in enumerate(result['confirmed_citations'], 1):
                ...
                if 'metadata' in citation and citation['metadata'].get('has_pin_cite'):
                    print(f"     Pin cite: {citation['metadata'].get('pin_cite', 'N/A')}")
            
            # Check expected results
            if 'expected_count' in test_case:
                if len(result['confirmed_citations']) == test_case['expected_count']:
                    print(f"✓ PASS: Found expected {test_case['expected_count']} citations")
                    passed += 1
                else:
                    print(f"✗ FAIL: Expected {test_case['expected_count']} citations, got {len(result['confirmed_citations'])}")
                    failed += 1
                    continue
            ...
            if 'expected' in test_case:
                ...
                if 'case_name' in test_case['expected']:
                    ...
                    if actual == expected:
                        ...
                        print(f"✗ FAIL: Case name mismatch. Expected: '{expected}', Got: '{actual}'")
                ...
                if 'citation_text' in test_case['expected']:
                    ...
                    if actual == expected:
                        ...
                        print(f"✗ FAIL: Citation text mismatch. Expected: '{expected}', Got: '{actual}'")
                        all_checks_passed = False
                ...
                if 'has_pin_cite' in test_case['expected']:
                    ...
                    if actual == expected:
                        ...
                        print(f"✗ FAIL: Pin cite mismatch. Expected: {expected}, Got: {actual}")
                        all_checks_passed = False
