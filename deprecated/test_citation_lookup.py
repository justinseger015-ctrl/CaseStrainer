...
def test_citation_lookup(citation):
    ...
    try:
        ...
        logger.info(
            f"- API Key: {'*' * 10}{os.getenv('COURTLISTENER_API_KEY', '')[-4:] if os.getenv('COURTLISTENER_API_KEY') else 'NOT SET'}"
        )
        ...
        logger.error(
            f"Unexpected error during citation lookup: {str(e)}", exc_info=True
        )
        return False
