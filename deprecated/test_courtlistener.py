...
if not API_KEY:
    ...
    logger.info(
        "Please set the COURTLISTENER_API_KEY in your .env file or environment variables"
    )
    sys.exit(1)
...
def check_courtlistener_api(citation_text):
    ...
    payload = {"text": citation_text, "type": "citation", "return_models": True}

    try:
        response = requests.post(
            f"{API_BASE_URL}{API_ENDPOINT}", headers=headers, json=payload, timeout=10
        )
        ...
        if isinstance(data, list):
            return {"status": "success", "results": data, "count": len(data)}
        return data
