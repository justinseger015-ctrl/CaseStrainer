...
class CitationTester:
    def __init__(self, base_url: str = BASE_URL):
        ...
        self.session.headers.update(
            {"User-Agent": "CaseStrainer-Test/1.0", "Accept": "application/json"}
        )

    def _make_request(
        self, endpoint: str, method: str = "POST", timeout: int = 30, **kwargs
    ) -> TestResult:
        ...
        try:
            ...
            if "headers" in kwargs and "Authorization" in kwargs["headers"]:
                ...
                logger.info(
                    f"Using Authorization header: {auth_header.split()[0]} {auth_header.split()[1][:6]}..."
                )
    ...
    def test_url_analysis(self, url: str) -> TestResult:
        print(f"\n🌐 Testing URL: {url}")
        return self._make_request(
            "analyze", "POST", json={"url": url, "source_type": "url"}, timeout=60
        )  # Longer timeout for URL analysis
...
def load_config():
    ...
    if not os.environ.get("COURTLISTENER_API_KEY"):
        if "COURTLISTENER_API_KEY" in config:
            ...
            "courtlistener_api_key" in config
            and config["courtlistener_api_key"] != "YOUR_API_KEY_HERE"
        ):
            os.environ["COURTLISTENER_API_KEY"] = config["courtlistener_api_key"]
...
def main():
    ...
    print("=" * 80)
    try:
        result = tester.test_url_analysis(
