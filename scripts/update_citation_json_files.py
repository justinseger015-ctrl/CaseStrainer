...
JSON_FILES = {
    ...
    "truly_unverified_citations.json": os.path.join(
        REPO_DIR, "truly_unverified_citations.json"
    ),
}
...
def get_citations_from_database():
    ...
    try:
        ...
        # Get all citations
        cursor.execute(
            "SELECT id, citation_text, case_name, confidence, found, explanation, source, source_document, url, context, date_added FROM citations"
        )
...
def update_verified_citations(citations):
    ...
    logger.info(
        f"Updated verified_citations.json with {len(verified['citations'])} citations"
    )
...
def update_verification_results(citations):
    ...
    for citation in citations:
        ...
        if source not in results["sources"]:
            results["sources"][source] = {"total": 0, "verified": 0, "unverified": 0}
    ...
    logger.info(
        f"Updated verification_results.json with results for {len(citations)} citations"
    )
...
def update_truly_unverified_citations(citations):
    ...
    for citation in citations:
        ...
        is_wa_citation = (
            ("WA" in citation["source"] if citation["source"] else False)
            or ("Wn." in citation["citation"] if citation["citation"] else False)
            or ("Wash." in citation["citation"] if citation["citation"] else False)
        )
    ...
    logger.info(
        f"Updated truly_unverified_citations.json with {len(truly_unverified)} citations"
    )
...
def copy_to_d_drive():
    ...
    try:
        ...
        if os.path.exists(d_drive_dir):
            ...
            logger.warning(
                f"D: drive directory {d_drive_dir} does not exist. Files not copied."
            )
