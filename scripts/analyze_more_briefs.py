    ...
    try:
        ...
        with open(output_path, "wb") as f:
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    f.write(chunk)
        print(f"Downloaded brief to {output_path}")
...
def extract_unconfirmed_citations(analysis_result, source_path=None):
    ...
    for citation in citation_results:
        ...
        if not citation.get("court_listener_url") or citation.get(
            ...
            unconfirmed.append(
                {
                    ...
                    "hallucination_status": citation.get(
...
def main():
    ...
    if os.path.exists(existing_file):
        try:
            ...
            print(
                f"Loaded {sum(len(citations) for citations in all_unconfirmed_citations.values())} existing unconfirmed citations"
            )
        except Exception as e:
    ...
    print(
        f"Currently have {current_count} unconfirmed citations. Target: {target_citations}"
    )

    # Process briefs from each court until we reach the target
    court_cycle = 0
    while (
        current_count < target_citations and court_cycle < 5
    ):  # Limit to 5 cycles through all courts
        court_cycle += 1
        ...
        for court_id, court_url in COURTS.items():
            ...
            for i, link in enumerate(brief_links[:briefs_to_process]):
                if current_count >= target_citations:
                    print(
                        f"Reached target of {target_citations} unconfirmed citations!"
                    )
                    break
            ...
            if downloaded_path:
                ...
                if os.path.exists(results_file):
                    print(
                        f"Analysis results already exist: {os.path.basename(results_file)}"
                    )
                ...
                if analysis_result:
                    ...
                    unconfirmed = extract_unconfirmed_citations(
                        analysis_result, downloaded_path
                    )
                    ...
                    if unconfirmed:
                        ...
                        print(
                            f"Total unconfirmed citations: {current_count}/{target_citations}"
                        )

                        # Print unconfirmed citations
                        for i, citation in enumerate(unconfirmed, 1):
                            print(
                                f"  {i}. {citation['citation_text']} - {citation['case_name']}"
                            )
            ...
            if all_unconfirmed_citations:
                intermediate_file = os.path.join(
                    DOWNLOAD_DIR, "all_unconfirmed_citations.json"
                )
                try:
                    with open(intermediate_file, "w", encoding="utf-8") as f:
                        json.dump(all_unconfirmed_citations, f, indent=2)
                    print(
                        f"Saved {current_count} unconfirmed citations to {intermediate_file}"
                    )
                except Exception as e:
    ...
    if all_unconfirmed_citations:
        ...
        for filename in all_unconfirmed_citations:
            court = next(
                (court_id for court_id in COURTS if court_id in filename.lower()),
                "unknown",
            )
            court_counts[court] = court_counts.get(court, 0) + len(
                all_unconfirmed_citations[filename]
            )
    ...
    print(
        f"The flattened citations file is at: {os.path.join(DOWNLOAD_DIR, 'unconfirmed_citations_flat.json')}"
    )
