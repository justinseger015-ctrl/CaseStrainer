#!/usr/bin/env python3
"""
Simple Real Briefs Test Script
==============================

This script processes real legal briefs using a simplified approach
to test the enhanced extraction system.
"""
...
class SimpleRealBriefsTester:
    """Test the enhanced extraction system on real legal briefs."""
    
    def __init__(self, briefs_dir: str = "../downloaded_briefs", max_briefs: int = 5):
    ...
    def extract_text_from_brief(self, pdf_path: Path) -> Optional[str]:
        ...
        try:
            ...
            text = extract_text_from_pdf_simple(str(pdf_path))
            
            if not text or len(text.strip()) < 100:
                print(f"   ⚠️  Warning: Minimal text extracted ({len(text) if text else 0} chars)")
                return None
    ...
    def process_brief_with_enhanced(self, text: str, brief_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self.enhanced_processor.process_text_enhanced(text)
    ...
    def run_tests(self):
        ...
        print("🚀 SIMPLE REAL BRIEFS TEST SUITE")
        print("=" * 80)
    ...
    def save_results(self):
        ...
        results_file = f"simple_real_briefs_test_results_{timestamp}.json"
...
def main():
    """Main function to run the simple real briefs test."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Test enhanced extraction on real briefs (simple version)')
    parser.add_argument('--briefs-dir', default='../downloaded_briefs', 
                       help='Directory containing brief PDFs')
    parser.add_argument('--max-briefs', type=int, default=5,
                       help='Maximum number of briefs to process')
    
    args = parser.parse_args()
    
    # Create and run tester
    tester = SimpleRealBriefsTester(
