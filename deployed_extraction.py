...
class UnifiedCaseExtractionMaster:
    ...
    def _extract_with_comma_anchor(self, text: str, citation: str, start_index: int, debug: bool) -> Optional[MasterExtractionResult]:
        ...
        # Standard: 400 chars, Subsequent history: 800 chars (to reach original case name)
        context_window = 800 if has_subsequent_history else 400
        ...
        for pattern, priority, pattern_type in patterns:
            for match in re.finditer(pattern, potential_case_name, re.IGNORECASE):
                case_name = match.group(1).strip()
                distance_from_end = len(potential_case_name) - match.end()
                
                # FIX #8: Check if this crosses a section header boundary
                text_after_match = potential_case_name[match.end():]
                crosses_boundary = bool(re.search(r'\n\s*[A-Z][A-Z\s]{3,}\n', text_after_match[:200]))
    ...
    def _clean_case_name(self, case_name: str, context: str = None) -> str:
        ...
        # Pattern 1: Before "v." - "Inc. No. 12-3456 78 v. Marston" → "Inc. v. Marston"
        cleaned = re.sub(r'\s+No\.\s+[\d\-]+\s+\d+(?=\s+v\.)', '', cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r'\s+No\.\s+[\d\-]+(?=\s+v\.)', '', cleaned, flags=re.IGNORECASE)
        
        # Pattern 1b: Enhanced pattern for complex case numbers like "No. 103430-0-15"
        # Handles: "Inc. No. 103430-0-15 v. Marston" → "Inc. v. Marston"
        cleaned = re.sub(r'\s+No\.\s+[\d\-]+\-[\d\-]+(?=\s+v\.)', '', cleaned, flags=re.IGNORECASE)
        
        # Pattern 2: After "v." - "Inc. v. Band 6 No. 103430-0 Tribe" → "Inc. v. Band Tribe"
        # This handles: "(quoting C&L Enters., Inc. v. Citizen Band 6 No. 103430-0 Potawatomi Tribe..."
        cleaned = re.sub(r'\s+\d+\s+No\.\s+[\d\-]+\s+', ' ', cleaned, flags=re.IGNORECASE)
        
        # Pattern 2b: Enhanced for complex case numbers after v.
        cleaned = re.sub(r'\s+No\.\s+[\d\-]+\-[\d\-]+\s+', ' ', cleaned, flags=re.IGNORECASE)
