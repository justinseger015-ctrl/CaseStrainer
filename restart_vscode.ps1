...
if ($vscodeProcesses) {
    ...
    Write-Host "`n🛑 Stopping VS Code processes..." -ForegroundColor Yellow
    $vscodeProcesses | Stop-Process -Force
    Start-Sleep -Seconds 2
}

# Verify configuration files exist
Write-Host "`n✅ Checking configuration files..." -ForegroundColor Green
$configFiles = @(
    "pyrightconfig.json",
...
Write-Host "`n📋 Current Pylance exclusions:" -ForegroundColor Cyan
...
# Start VS Code
Write-Host "`n🚀 Starting VS Code..." -ForegroundColor Green
...
Write-Host "`n✅ VS Code restart complete!" -ForegroundColor Green 