<template>
  <div class="citation-results">
    ...
    <div v-else>
      <!-- Summary Section -->
      <div v-if="results && results.metadata" class="card mt-4">
        <div class="card-header">
          <h5>Document Metadata</h5>
        </div>
        <div class="card-body">
          <ul class="list-group list-group-flush">
            <li class="list-group-item" v-for="(value, key) in displayableMetadata" :key="key">
...
export default {
  ...
  methods: {
    ...
    validateCitationData() {
      ...
      // Check required fields in each citation
      ...
      const citationErrors = citations.map((citation, index) => {
        const missingFields = requiredCitationFields.filter(field => {
          const hasField = field in citation;
          if (!hasField) {
            console.warn(`Citation ${index} missing required field: ${field}`, citation);
          }
          return !hasField;
        });
        
        if (missingFields.length > 0) {
          return `Citation ${index + 1} missing required fields: ${missingFields.join(', ')}`;
        }
        
        // Validate metadata structure
        if (!citation.metadata || typeof citation.metadata !== 'object') {
          return `Citation ${index + 1} has invalid metadata structure`;
        }
        
        const requiredMetadataFields = ['url', 'source', 'verified', 'timestamp'];
        const missingMetadataFields = requiredMetadataFields.filter(field => !(field in citation.metadata));
        
        if (missingMetadataFields.length > 0) {
          return `Citation ${index + 1} missing required metadata fields: ${missingMetadataFields.join(', ')}`;
        }
      ...
      if (citationErrors.length > 0) {
        this.validationErrors.push(...citationErrors);
        return false;
      }
    ...
    updateProcessedCitations() {
      console.group('CitationResults - updateProcessedCitations');
      try {
        this.validationErrors = [];
        console.log('Input results:', this.results);
        ...
        if (!citations.length) {
          console.warn('No citations found in results:', this.results);
        ...
        // Process citations
        const processed = citations.map((citation, index) => {
          console.log(`Processing citation ${index}:`, citation);
          
          if (!citation) {
            console.warn(`Citation ${index} is null or undefined`);
            return null;
          }
          ...
          console.log(`Processed citation ${index}:`, safeCitation);
          
          // Check for duplicates
          const cacheKey = `${safeCitation.citation}-${safeCitation.case_name}`;
          if (citationCache.has(cacheKey)) {
            console.log(`Citation ${index} is a duplicate of ${citationCache.get(cacheKey)}`);
            return null;
          }
          
          citationCache.set(cacheKey, index);
          return safeCitation;
        }).filter(Boolean);
        
        console.log('Final processed citations:', processed);
    ...
    formattedDebugInfo() {
      try {
        ...
        return 'Error formatting debug information';
      }
    },
    
    formattedProcessedCitations() {
      try {
        return JSON.stringify(this.processedCitationsCache, null, 2);
      } catch (error) {
        console.error('Error formatting processed citations:', error);
        return 'Error formatting processed citations';
      }
    }
  },
    ...
    results: {
      ...
      validator: (value) => {
        return value && (
          ...
          (typeof value === 'object' && value.citation_text)
        );
      }
    },
  ...
  computed: {
    isValidResults() {
      return this.results && (
        (Array.isArray(this.results.citations) && this.results.citations.length > 0) ||
        (typeof this.results === 'object' && this.results.citation_text)
      );
    },
    citations() {
      if (!this.isValidResults) return [];
      return Array.isArray(this.results.citations) ? this.results.citations : [this.results];
    },
    totalCitations() {
      return this.results?.total_citations || this.citations.length;
    },
    verifiedCount() {
      return this.results?.verified_count || this.citations.filter(c => c.verified).length;
    },
    unverifiedCount() {
      return this.results?.unverified_count || this.citations.filter(c => !c.verified).length;
    },
    courtListenerCount() {
      return this.results?.courtlistener_count || this.citations.filter(c => c.source === 'CourtListener').length;
    },
    otherVerifiedCount() {
      return this.results?.other_verified_count || this.citations.filter(c => c.verified && c.source !== 'CourtListener').length;
    },
    multitoolCount() {
      return this.citations.filter(c => c.validation_method === 'multi-tool').length;
    },
    activeTabContent() {
      switch (this.activeTab) {
        case 'courtlistener':
          return this.citations.filter(c => c.source === 'CourtListener');
        case 'other':
          return this.citations.filter(c => c.verified && c.source !== 'CourtListener');
        case 'unverified':
    ...
    summaryAlertClass() {
      if (this.verifiedCount === this.totalCitations) return 'alert-success';
      if (this.unverifiedCount === this.totalCitations) return 'alert-warning';
      return 'alert-info';
    },
    summaryIcon() {
      if (this.verifiedCount === this.totalCitations) return 'fa-check-circle';
      if (this.unverifiedCount === this.totalCitations) return 'fa-exclamation-triangle';
      return 'fa-info-circle';
    },
    displayableMetadata() {
      // Harmonize and format metadata fields for display
      const meta = this.results?.metadata || {};
      const mapping = {
        'Source Type': meta.source_type || meta.source || '-',
        'Source Name': meta.source_name || meta.fileName || meta.url || '-',
        'Length': meta.length || meta.fileSize || meta.content_length || '-',
        'Processing Time (s)': meta.processing_time || meta.execution_time || '-',
        'Timestamp': meta.timestamp || '-',
        'File Type': meta.file_type || meta.fileType || '-',
        'Content Type': meta.content_type || '-',
        'User Agent': meta.user_agent || '-',
        'Verified': meta.verified !== undefined ? String(meta.verified) : '-',
      };
      // Remove empty fields
      return Object.fromEntries(Object.entries(mapping).filter(([k, v]) => v && v !== '-'));
    },
  },
