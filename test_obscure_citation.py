#!/usr/bin/env python3
"""
Test with an obscure citation to trigger reporter-first verification
"""
...
def test_obscure_citation():
    """Test with an obscure citation that might fail primary lookup but succeed with reporter-first"""
    
    # Try an older state reporter citation that might not be in CourtListener's main database
    obscure_citation = "474 A.2d 191"  # This is from the PDF - let's see if we can trigger reporter-first
    
    test_text = f"In the case of {obscure_citation}, the court held that..."
    
    request_data = {
        "type": "text",
        "text": test_text,
        "client_request_id": f"test_obscure_{int(time.time())}"
    }
    
    print(f"Testing obscure citation: {obscure_citation}")
            ...
            if citations:
                ...
                if clusters:
                    ...
                    if cluster_source and "reporter-first" in cluster_source.lower():
                        ...
                        print(f"  ⚠️  Verified by source: {cluster_source}")
                    else:
