...
def analyze():
    ...
    if request.method == "POST":
        ...
        if "briefText" in request.form:
            ...
            threading.Thread(
                target=run_analysis, args=(analysis_id, brief_text)
            ).start()
            ...
            return (
                jsonify({"status": "error", "message": "No brief text provided"}),
                400,
            )
    else:
        ...
        return (
            jsonify(
                {
                    "status": "error",
                    "message": "Use POST to start analysis and GET /analyze_status to check status",
                }
            ),
            400,
        )
