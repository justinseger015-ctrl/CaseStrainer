#!/usr/bin/env python3
"""
BriefCheck Demo (Non-interactive)
...
def main():
    ...
    print(
        "In a real implementation, you would need to install the dependencies listed in requirements.txt."
    )

    # Use sample text automatically
    ...
    if results["hallucinated_citations"] > 0:
        ...
        for result in results["results"]:
            if result["is_hallucinated"]:
                print(
                    f"- {result['citation']} (Confidence: {result['confidence']:.2f}, Similarity: {result['similarity_score']:.2f})"
                )
    ...
    for i, result in enumerate(results["results"]):
        ...
        print(
            f"{i+1}. {result['citation']} - {status} (Similarity: {result['similarity_score']:.2f})"
        )
        print("   Summaries:")
