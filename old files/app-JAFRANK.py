...
def analyze():
    ...
    try:
        ...
        print(
            f"Request files: {list(request.files.keys()) if request.files else 'None'}"
        )
            ...
            try:
                ...
                if file_extension == ".docx":
                    ...
                    try:
                        ...
                        brief_text = text
                        print(
                            f"Total extracted text length: {len(brief_text)} characters"
                        )
                    except Exception as e:
                    ...
                    try:
                        ...
                        return (
                            jsonify({"error": f"Failed to read text file: {str(e)}"}),
                            400,
                        )
            finally:
        ...
        try:
            ...
            use_local_pdf_search = (
                request.form.get("use_local_pdf_search", "false").lower() == "true"
            )
                            ...
                            if message["status"] == "complete":
                                ...
                                if message["status"] == "result":
                                    if message["result"].get("is_hallucinated", False):
                                        hallucinated_count = message.get(
...
def extract():
    ...
    try:
        ...
        if not brief_text:
            return jsonify({"error": "No text provided for citation extraction."}), 400
