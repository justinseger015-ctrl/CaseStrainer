...
try:
    with open(import_path, 'r') as f:
        config = json.load(f)
        imported_key = config.get('COURTLISTENER_API_KEY') or config.get('courtlistener_api_key')
...
if imported_key:
    print(f"Loaded CourtListener API key from config.json: {API_KEY[:6]}... (truncated)")
elif os.environ.get('COURTLISTENER_API_KEY'):
    print(f"Loaded CourtListener API key from environment variable: {API_KEY[:6]}... (truncated)")
else:
    print("No valid CourtListener API key found! Please set it in config.json or as an environment variable.")
