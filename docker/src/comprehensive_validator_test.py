...
def test_citation_suggestions(citation):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Suggestions API error: {response.status_code} - {response.text}")
            return None
...
def run_comprehensive_test(citation, citation_type):
    ...
    if context_result:
        ...
        results["context_available"] = context and context != f'No context available for {citation}'
        results["context_length"] = len(context)
        print(f"  Context: {'✓ Available' if results['context_available'] else '✗ Not available'} ({results['context_length']} chars)")
...
def generate_report(all_results):
    ...
    for result in all_results:
        ...
        stats_by_type[citation_type]["context_available"] += 1 if result["context_available"] else 0
        stats_by_type[citation_type]["avg_confidence"] += result["ml_confidence"]
        stats_by_type[citation_type]["avg_suggestion_count"] += result["suggestion_count"]
    ...
    print(tabulate(
        summary_table,
        headers=["Citation Type", "Total", "Validated", "Context Available", "Avg. Confidence", "Avg. Suggestions"],
        tablefmt="grid"
    ))
...
def generate_visualizations(stats_by_type, timestamp):
    ...
    validation_rates = [stats_by_type[t]["validation_rate"] * 100 for t in citation_types]
    context_rates = [stats_by_type[t]["context_rate"] * 100 for t in citation_types]
    confidence_scores = [stats_by_type[t]["avg_confidence"] * 100 for t in citation_types]
    ...
    plt.subplot(2, 1, 2)
    suggestion_counts = [stats_by_type[t]["avg_suggestion_count"] for t in citation_types]
