...
def verify_citation(citation):
    ...
    try:
        ...
        logger.info(
            f"[verify_citation] Using API key: {api_key[:6]}... (length: {len(api_key)})"
        )
    ...
    try:
        ...
        if EYECITE_AVAILABLE and tokenizer:
            ...
            try:
                ...
                logger.warning(
                    f"Error using AhocorasickTokenizer: {e}. Falling back to regex patterns."
                )
                citations = None
        else:
        ...
        if citations:
            ...
            for citation in citations:
                ...
                if re.search(
                    r"\d+\s+U\.\s*S\.\s*C\.\s*§", citation_text, re.IGNORECASE
                ):
                ...
                citation_text = re.sub(
                    r"([0-9])\s+([A-Z])", r"\1 \2", citation_text
                )  # Fix spacing in citations
                citation_text = re.sub(
                    r"([A-Z])\.\s+([A-Z])", r"\1. \2", citation_text
        ...
        if not cleaned_citations:
            ...
            for pattern in scotus_patterns:
                ...
                for match in matches:
                    ...
                    citation = re.sub(
                        r"([0-9])\s+([A-Z])", r"\1 \2", citation
                    )  # Fix spacing
                    citation = re.sub(
                        r"([A-Z])\.\s+([A-Z])", r"\1. \2", citation
                    )  # Fix abbreviations
