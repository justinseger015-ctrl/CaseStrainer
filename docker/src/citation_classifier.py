...
def extract_features_from_citation(citation_text, case_name=None):
    ...
    features['has_court_abbreviation'] = 1 if re.search(r'\b(S\.\s*Ct\.|F\.\s*\d+d|U\.\s*S\.|P\.\s*\d+d|Wn\.\s*\d+d|Wn\.\s*App\.)', citation_text, re.IGNORECASE) else 0
    ...
    if reporter_match:
        ...
        common_reporters = ['f.', 'f.2d', 'f.3d', 'u.s.', 's.ct.', 'l.ed.', 'p.', 'p.2d', 'p.3d', 
                           'wn.', 'wn.2d', 'wn.app.', 'wash.', 'wash.2d', 'wash.app.']
...
def train_citation_classifier():
    ...
    if len(citations) < 50:
        print(f"Not enough citation data to train a model. Found only {len(citations)} citations.")
        return None, None
