...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'wa_citations_extraction_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def find_citations(text: str) -> Set[str]:
    """Find all citations in the text using eyecite and additional patterns."""
    citations = set()
    try:
