...
def save_processed_cases(processed_cases):
    ...
    try:
        ...
        logger.info(f"Saved {len(processed_cases)} processed cases to {PROCESSED_CASES_CACHE}")
    except Exception as e:
...
def process_court(court_url, processed_cases, downloaded_briefs, total_cases_processed):
    ...
    for case in new_cases:
        ...
        if total_cases_processed >= MAX_TOTAL_CASES:
            logger.info(f"Reached limit of {MAX_TOTAL_CASES} cases processed. Stopping.")
...
def save_downloaded_briefs(downloaded_briefs):
    ...
    try:
        ...
        logger.info(f"Saved metadata for {len(downloaded_briefs)} briefs to {metadata_path}")
    except Exception as e:
...
def main():
    ...
    if os.path.exists(metadata_path):
        try:
            ...
            logger.info(f"Loaded metadata for {len(downloaded_briefs)} previously downloaded briefs")
        except Exception as e:
    ...
    for court_url in COURT_URLS:
        total_cases_processed = process_court(court_url, processed_cases, downloaded_briefs, total_cases_processed)
    ...
    newly_downloaded = sum(1 for brief in downloaded_briefs if brief.get('downloaded', False))
    logger.info(f"Downloaded {newly_downloaded} new briefs from {total_cases_processed} cases")
