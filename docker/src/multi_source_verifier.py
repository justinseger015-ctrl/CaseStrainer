...
VERIFICATION_SOURCES = {
    ...
    "justia": {
        "base_url": "https://www.justia.com/search",
        "requires_api_key": False
    },
    "leagle": {
        "base_url": "https://www.leagle.com/search",
        "requires_api_key": False
    },
    "findlaw": {
    ...
    "casetext": {
        "base_url": "https://casetext.com/search",
        "requires_api_key": False
    }
}
    ...
    def verify_citation(self, citation_text, case_name=None, sources=None):
        ...
        for source in sources_to_check:
            try:
                ...
                traceback.print_exc()
                self.verification_results[source] = {
                    "found": False,
                    "error": str(e)
                }
        ...
        likelihood_score = self.calculate_likelihood_score(normalized_citation, case_name, format_analysis)
            ...
            if match:
                ...
                if format_type == "us_reports":
                    ...
                    if not is_valid_volume:
                        result["volume_error"] = f"Volume {volume_num} is outside the valid range ({min_vol}-{max_vol}) for {format_type}"
                    ...
                    if not is_valid_volume:
                        result["volume_error"] = f"Volume {volume_num} is outside the valid range ({min_vol}-{max_vol}) for {format_type} series {series_num}"
                ...
                return {
                    ...
                    "details": {"note": "Format recognized but detailed validation not implemented"}
                }
...
class MultiSourceVerifier:
    ...
    def calculate_likelihood_score(self, citation_text, case_name, format_analysis):
        ...
        if case_name:
            ...
            legal_terms = ["state", "united states", "city", "county", "department", "board", "commission", "corporation"]
    ...
    def check_courtlistener(self, citation_text, case_name=None):
        ...
        """
        # Get API key
        api_key = self.api_keys.get("courtlistener") or os.environ.get("COURTLISTENER_API_KEY")
        if not api_key:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("count", 0) > 0:
                    ...
                    name_match = True
                    if case_name:
                        name_match = check_name_match(case_name, result.get("case_name", ""))
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if results:
                    ...
                    # Get URL
                    url = title.select_one('a')['href'] if title and title.select_one('a') else ""
...
if __name__ == "__main__":
    ...
    test_citations = [
        ...
        {"citation": "2019 WL 6686274", "case_name": "Martinez v. Department of Corrections"}
    ]
