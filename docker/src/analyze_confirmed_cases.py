...
def main():
    ...
    for i, case in enumerate(confirmed_cases, 1):
        ...
        if unconfirmed:
            print(f"Found {len(unconfirmed)} unconfirmed citations in confirmed case {case_id}")
        ...
        if cases_with_unconfirmed:
            results_file = os.path.join(CONFIRMED_DIR, 'confirmed_cases_with_unconfirmed.json')
            with open(results_file, 'w', encoding='utf-8') as f:
    ...
    total_unconfirmed = sum(len(case_data['unconfirmed_citations']) for case_data in cases_with_unconfirmed.values())
    
    print(f"Analyzed {len(confirmed_cases)} confirmed cases")
    print(f"Found {total_unconfirmed} unconfirmed citations in {total_cases_with_unconfirmed} confirmed cases")
    ...
    if cases_with_unconfirmed:
        ...
        for case_id, case_data in cases_with_unconfirmed.items():
            for citation in case_data['unconfirmed_citations']:
                ...
                citation['source_file'] = f"confirmed_case_{case_id}.txt"  # Create a source filename for display
                flattened_citations.append(citation)
        
        flattened_file = os.path.join(CONFIRMED_DIR, 'confirmed_cases_unconfirmed_citations_flat.json')
        ...
        try:
            ...
            existing_file = os.path.join(DOWNLOAD_DIR, 'unconfirmed_citations_flat.json')
            if os.path.exists(existing_file):
                with open(existing_file, 'r', encoding='utf-8') as f:
                    existing_citations = json.load(f)
                print(f"Loaded {len(existing_citations)} existing unconfirmed citations")
                
                # Merge the two lists
                combined_citations = existing_citations + flattened_citations
                print(f"Combined total: {len(combined_citations)} unconfirmed citations")
                
                # Save the combined list
                with open(existing_file, 'w', encoding='utf-8') as f:
                    json.dump(combined_citations, f, indent=2)
                print(f"Added {len(flattened_citations)} new unconfirmed citations to the main database")
            else:
                print(f"No existing unconfirmed citations file found at {existing_file}")
                # Copy our flattened file to the main location
                import shutil
                shutil.copy(flattened_file, existing_file)
                print(f"Created new unconfirmed citations database with {len(flattened_citations)} citations")
        except Exception as e:
