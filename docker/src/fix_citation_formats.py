...
def save_corrected_citations(citations, filename='downloaded_briefs/corrected_citations.json'):
...
def analyze_citation_format(citation_text):
    ...
    if us_match:
        ...
        if int(volume) > 600:
            return (False, "U.S. Reports", f"{volume} U.S. {page}", "Invalid volume number (too high)")
        # Standardize format
    ...
    if f_match:
        ...
        if int(series) > 4:
            return (False, "Federal Reporter", f"{volume} F.{series}d {page}", "Invalid series number (too high)")
        # Standardize format
    ...
    if fsupp_match:
        ...
        if int(series) > 3:
            return (False, "Federal Supplement", f"{volume} F.Supp.{series}d {page}", "Invalid series number (too high)")
        # Standardize format
    ...
    if wash_match:
        ...
        # Check if series is valid (1, 2, 3)
        if int(series) > 3:
            return (False, "Washington Reports", f"{volume} {state_abbr}.{series}d {page}", "Invalid series number (too high)")
        # Standardize format
    ...
    wash_app_match = re.match(r'^(\d+)\s+(Wn\.|Wash\.|Wash|Wn)\s+App\.\s+(\d+)$', citation)
    if wash_app_match:
        ...
        if corrected != citation:
            return (True, "Washington Appellate Reports", corrected, "Standardized format")
    ...
    if sct_match:
        ...
        if int(volume) > 150:
            return (False, "Supreme Court Reporter", f"{volume} S.Ct. {page}", "Invalid volume number (too high)")
        # Standardize format
    ...
    if wl_match:
        ...
        if int(year) < 1980 or int(year) > current_year:
            return (False, "Westlaw", f"{year} WL {number}", f"Invalid year (should be between 1980 and {current_year})")
        # Standardize format
...
def fix_citation_formats():
    ...
    for document, citations in all_citations.items():
        ...
        for citation in citations:
            ...
            is_valid, format_type, correction, message = analyze_citation_format(citation_text)
            ...
            if not is_valid:
                ...
                suggested_citation = suggest_real_citation(case_name, citation_text) if case_name else None
