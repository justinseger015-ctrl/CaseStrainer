...
def load_enhanced_citations(
    filename="downloaded_briefs/enhanced_unconfirmed_citations.json",
):
...
def analyze_citations_by_type():
    ...
    if not enhanced_citations:
        print(
            "No enhanced citations found. Please run scan_and_categorize_citations.py first."
        )
        return
    ...
    for format_type, citations in citations_by_type.items():
        ...
        valid_format_count = sum(
            1 for c in citations if c.get("is_valid_format", False)
        )
        valid_volume_count = sum(
            1 for c in citations if c.get("is_valid_volume", False)
        )

        # Calculate average likelihood
        avg_likelihood = sum(
            c.get("likelihood_of_being_real", 0) for c in citations
        ) / len(citations)
...
def generate_report(type_stats, flat_citations):
    ...
    with open(report_file, "w") as f:
        ...
        # Overall statistics
        valid_format_count = sum(
            1 for c in flat_citations if c.get("is_valid_format", False)
        )
        valid_volume_count = sum(
            1 for c in flat_citations if c.get("is_valid_volume", False)
        )

        f.write("=== OVERALL STATISTICS ===\n")
        f.write(
            f"Valid Formats: {valid_format_count} ({valid_format_count/len(flat_citations)*100:.1f}%)\n"
        )
        f.write(
            f"Valid Volumes: {valid_volume_count} ({valid_volume_count/len(flat_citations)*100:.1f}%)\n\n"
        )
        ...
        for format_type, stats in sorted_types:
            f.write(f"\n--- {format_type.upper()} ---\n")
            f.write(
                f"Count: {stats['count']} ({stats['count']/len(flat_citations)*100:.1f}%)\n"
            )
            f.write(
                f"Valid Formats: {stats['valid_format_count']} ({stats['valid_format_percent']:.1f}%)\n"
            )
            f.write(
                f"Valid Volumes: {stats['valid_volume_count']} ({stats['valid_volume_percent']:.1f}%)\n"
            )
    ...
    try:
        ...
        if type_stats:
            ...
            for format_type, stats in sorted(
                ...
                print(
                    f"  {format_type}: {stats['count']} citations, {stats['valid_format_percent']:.1f}% valid format, {stats['valid_volume_percent']:.1f}% valid volume"
                )
