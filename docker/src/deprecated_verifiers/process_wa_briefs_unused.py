...
def process_briefs(briefs_dir, metadata_file=None):
    ...
    for brief in tqdm(briefs, desc="Processing briefs"):
        ...
        for citation in tqdm(
            citations, desc=f"Processing citations in {brief['id']}", leave=False
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Process Washington State Court Briefs for Citations"
    )
    parser.add_argument(
        "--briefs_dir",
        type=str,
        default="wa_briefs",
        help="Directory containing downloaded briefs",
    )
    parser.add_argument(
        "--metadata_file",
        type=str,
        default=None,
        help="Path to briefs metadata JSON file",
    )
