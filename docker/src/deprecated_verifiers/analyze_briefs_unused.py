...
from app_final import extract_citations
    ...
    print("Error: Could not import CaseStrainer functions. Make sure you're running this script from the CaseStrainer directory.")
    sys.exit(1)
...
def update_briefs_csv(briefs):
    ...
    try:
        with open(BRIEFS_CSV, 'w', newline='', encoding='utf-8') as f:
            fieldnames = ['Docket Number', 'Caption', 'Brief Type', 'Filing Date', 'PDF URL', 'Local Path', 'Analyzed', 'Unverified Citations']
...
def analyze_brief(brief, verifier):
    ...
    try:
        ...
        brief_text_result = extract_text_from_file(local_path)
        ...
        print(f"Found {len(unverified_citations)} unverified citations in brief: {brief['Caption']}")
