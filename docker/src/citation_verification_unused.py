...
class CitationVerifier:
                        ...
                        # Check if the response has citations (older API format)
                            ...
                            for citation_result in api_result["citations"]:
                                if citation_result.get("found", False):
                                    result["found"] = True
                                    result["case_name"] = citation_result.get(
                                        "case_name", "Unknown Case"
                                    )
                                    result["url"] = citation_result.get("url", "")
                                    result["details"] = {
                                        "court": citation_result.get(
                                            "court", "Unknown Court"
                                        ),
                                        "year": citation_result.get(
                                            "year", "Unknown Year"
                                        ),
                                        "citation": citation_result.get(
                                            "citation", citation
                                        ),
                                    }
                                    return result
    ...
    def _format_citation_for_courtlistener(self, citation: str) -> str:
        ...
        if re.search(wash_2d_pattern, citation, re.IGNORECASE):
            return re.sub(
                wash_2d_pattern, r"\1 Wash. 2d \2", citation, flags=re.IGNORECASE
            )
            ...
            return re.sub(
                wash_app_pattern, r"\1 Wash. App. \2", citation, flags=re.IGNORECASE
            )
