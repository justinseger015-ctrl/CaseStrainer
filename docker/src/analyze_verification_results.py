...
def update_database_with_results(results):
    ...
    if 'verification_status' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_citations ADD COLUMN verification_status TEXT")
    
    if 'verification_confidence' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_citations ADD COLUMN verification_confidence REAL")
    
    if 'verification_source' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_citations ADD COLUMN verification_source TEXT")
    
    if 'verification_tag' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_citations ADD COLUMN verification_tag TEXT")
...
def analyze_results(results):
    ...
    verified_citations = sum(1 for r in results if r.get('verification_result', {}).get('found', False))
    ...
    analysis = {
        ...
        'multi_source_rate': multi_source_only / verified_citations if verified_citations > 0 else 0,
        'sources': sources,
    ...
    print(f"Verified citations: {verified_citations} ({analysis['verification_rate']:.2%})")
    print(f"Unverified citations: {unverified_citations}")
    print(f"Multi-source only verifications: {multi_source_only} ({analysis['multi_source_rate']:.2%} of verified)")
...
def generate_report(analysis, results):
    ...
    try:
        ...
        df['confidence'] = df['verification_result'].apply(lambda x: x.get('confidence', 0.0))
        df['source'] = df['verification_result'].apply(lambda x: x.get('source', 'unknown'))
        
        # Create visualizations
        plt.figure(figsize=(10, 6))
        plt.pie([analysis['verified_citations'], analysis['unverified_citations']], 
        ...
        for r in results:
            ...
            is_multi_source_only = found and source != 'CourtListener' and source != 'unknown'
