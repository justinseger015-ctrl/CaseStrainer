...
if not os.path.exists(EXTRACTED_DIR):
    try:
        ...
        try:
            import subprocess
            subprocess.run(["powershell", "-Command", f"New-Item -ItemType Directory -Path '{EXTRACTED_DIR}' -Force"], check=True)
    ...
    try:
        ...
        if existing:
            ...
            cursor.execute(
                ...
                (source, verification_result.get("verified", False), datetime.now().isoformat(), existing[0])
            )
            ...
            cursor.execute(
                ...
                (citation, source, verification_result.get("verified", False), datetime.now().isoformat())
            )
...
def main():
    ...
    for brief_info in metadata:
        ...
        if result:
            ...
            all_unverified_citations.extend(result.get("citations", {}).get("unverified", []))
