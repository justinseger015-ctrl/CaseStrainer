...
def check_case_with_langsearch(citation_text):
    ...
    try:
        ...
        if (
            "unable to find" in second_summary.lower()
            or "no information" in second_summary.lower()
            or "not a valid" in second_summary.lower()
        ):
            return {
                "is_real": False,
                "confidence": 0.8,
...
def query_courtlistener_api(text, api_key, timeout=30):
    """Query the CourtListener API to verify citations in the text.

    Args:
        text (str): The text containing citations to verify
        api_key (str): The CourtListener API key
        timeout (int): Maximum time in seconds to wait for a response (default: 30)
    ...
    try:
        ...
        print(
            f"Sending request to {COURTLISTENER_API_URL} with timeout of {timeout} seconds"
        )
        try:
            response = requests.post(
                COURTLISTENER_API_URL, headers=headers, json=data, timeout=timeout
            )
            ...
            error_msg = f"CourtListener API request timed out after {timeout} seconds"
            print(error_msg)
            return {"error": error_msg, "timed_out": True}

    except Exception as e:
        error_msg = f"Error querying CourtListener API: {str(e)}"
        print(error_msg)
        traceback.print_exc()
        return {"error": error_msg}


def validate_single_citation(citation, api_key, timeout=30):
    """Validate a single citation with a timeout.
    ...
    Returns:
        dict: The validation result for this citation
    """
    try:
        print(f"Validating citation: {citation}")
        # Call the API with just this citation
        result = query_courtlistener_api(citation, api_key, timeout=timeout)
        ...
        if "error" in result:
            if result.get("timed_out"):
                return {
                    ...
                    "confidence": 0.7,
                    "explanation": f"Citation validation timed out after {timeout} seconds",
                }
            else:
                return {
                    "citation_text": citation,
                    "is_hallucinated": True,
                    "confidence": 0.7,
                    "explanation": f"Error validating citation: {result['error']}",
                }

        # Process the API response for this single citation
        # This is a simplified version of the processing in run_analysis
        if isinstance(result, list):
            for item in result:
                if (
                    isinstance(item, dict)
                    and "citation" in item
                    and citation.lower() in item["citation"].lower()
                ):
                    # Found a match in the API response
                    court_listener_url = None
                    case_name = "Unknown case"

                    if "clusters" in item and item["clusters"]:
                        cluster = item["clusters"][0]
                        court_listener_url = cluster.get("absolute_url", "")
                        if court_listener_url and not court_listener_url.startswith(
                            "http"
                        ):
                            court_listener_url = (
                                f"https://www.courtlistener.com{court_listener_url}"
                            )
                    ...
                    return {
                        ...
                        "confidence": 0.9,
                        "explanation": f"Citation confirmed: {case_name}"
                        + (f" - {court_listener_url}" if court_listener_url else ""),
                        "court_listener_url": court_listener_url,
                        "case_name": case_name,
                    }

        # If we get here, the citation wasn't found in the API response
        return {
            "citation_text": citation,
            "is_hallucinated": True,
            "confidence": 0.7,
        ...
        error_msg = f"Error validating citation {citation}: {str(e)}"
        print(error_msg)
        traceback.print_exc()
        return {
            "citation_text": citation,
            "is_hallucinated": True,
            "confidence": 0.7,
            "explanation": f"Error validating citation: {str(e)}",
        }
    ...
    try:
        ...
        if api_key:
            ...
            for result in citation_results:
                ...
                if (
                    case_key
                    and case_key.startswith("http")
                    and case_name
                    and case_name != "Unknown case"
                    and not result["is_hallucinated"]
                ):
                    ...
                    if case_key in grouped_citations:
                        ...
                        grouped_citations[case_key]["parallel_citations"].append(
                        ...
                        if "summaries" in result:
                            grouped_citations[case_key]["summaries"] = result[
                                "summaries"
                            ]
                else:
                    ...
                    # Add any summaries if available
                    if "summaries" in result:
                        grouped_citations[citation_key]["summaries"] = result[
                            "summaries"
                        ]
            ...
            analysis_results[analysis_id][
                "message"
            ] = f"Analysis complete. Found {len(citations)} citations, {hallucinated_count} potentially hallucinated."
            analysis_results[analysis_id]["completed"] = True
