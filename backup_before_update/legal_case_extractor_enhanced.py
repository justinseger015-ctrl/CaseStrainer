...
if __name__ == "__main__":
    ...
    for case in cases:
        ...
        if case.date_info:
            ...
            if case.date_info.month:
                print(f"Full Date: {case.date_info.month} {case.date_info.day}, {case.date_info.year}")
            if case.date_info.court:
                print(f"Court: {case.date_info.court}")
                print(f"Court (normalized): {extractor._normalize_court_name(case.date_info.court)}")
        print("-" * 70)
    ...
    print(f"\n--- CSV Export Sample ---")
    if csv_data:
        print("Headers:", list(csv_data[0].keys()))
        print("First row:", {k: v for k, v in csv_data[0].items() if v is not None}) 