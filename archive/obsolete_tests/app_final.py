from flask import Flask, request, render_template, jsonify, Response, send_from_directory, redirect
...
try:
    from enhanced_validator_production import extract_citations, validate_citation, extract_text_from_file
...
try:
    ...
    print("Warning: citation_correction module not found. Correction suggestions will not be available.")
    def get_correction_suggestions(citation_text):
        return []

# Import citation classifier module
try:
    from citation_classifier import classify_citation, batch_classify_citations, train_citation_classifier
except ImportError:
    print("Warning: citation_classifier module not found. ML classification will not be available.")
...
try:
    ...
    print("Warning: citation_export module not found. Enhanced export functionality will not be available.")
...
try:
    ...
    print("Warning: enhanced_citation_verifier module not found. Enhanced verification will not be available.")
...
try:
    ...
    print("Warning: landmark_cases module not found. Landmark case verification will not be available.")
...
try:
    ...
    print("Warning: multi_source_verifier module not found. Enhanced verification will not be available.")
    USE_MULTI_SOURCE_VERIFIER = False
...
def search_citation_on_web(citation_text, case_name=None):
    ...
    if cached_result:
        ...
        if case_name and 'found_case_name' in cached_result:
            cached_result['name_match'] = check_name_match(case_name, cached_result['found_case_name'])
    ...
    if landmark_info:
        ...
        result = {
            'found': True,
            'url': f"https://supreme.justia.com/cases/federal/us/{citation_text.split()[0]}/{citation_text.split()[2]}/" if 'U.S.' in citation_text else None,
    ...
    def check_name_match(found_name, original_name):
        ...
        def normalize_case_name(name):
            ...
            common_terms = ['v.', 'vs.', 'versus', 'v', 'et al.', 'et al', 'inc.', 'inc', 'llc', 'corp.', 'corp',
                           'ltd.', 'ltd', 'co.', 'co', 'company', 'corporation', 'incorporated', 'limited',
                           'the', 'and', 'of', 'for', 'in', 're:', 're', 'matter of', 'estate of']
        ...
        for word in words_found:
            ...
            elif any(word in case_word or case_word in word for case_word in words_case if len(word) > 5 and len(case_word) > 5):
        ...
        match_percentage = matches / (min_words * 2)  # Normalize to account for exact matches worth 2
    ...
    try:
                    ...
                    for result_item in results:
                        ...
                        if clean_citation in result_text:
                            ...
                            if case_link:
                                ...
                                case_url = "https://www.leagle.com" + case_link['href'] if case_link['href'].startswith('/') else case_link['href']
        ...
        try:
            ...
            if justia_search_url:
                ...
                if response.status_code == 200:
                    ...
                    results = soup.select('.result-title') or soup.select('.gs-title') or soup.select('h3.title')
                    
                    for result_item in results:
                        result_text = result_item.text.strip()
                        # Check if the citation or parts of it appear in the result
                        if clean_citation in result_text or (('WL' in clean_citation) and clean_citation.split()[0] in result_text):
                            case_link = result_item.find('a') or result_item
                            if case_link and hasattr(case_link, 'href') and case_link['href']:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                results = soup.select('.search-result') or soup.select('.result-title') or soup.select('h3.title')
        ...
        if 'S.Ct.' in clean_citation or 'S. Ct.' in clean_citation:
            try:
                ...
                if match:
                    ...
                    direct_url = f"https://www.leagle.com/decision/in-sco-{volume}sct{page}-1"
                    ...
                    justia_direct_url = f"https://supreme.justia.com/cases/federal/us/{volume}/{page}/"
                    print(f"Trying direct URL for Supreme Court citation on Justia: {justia_direct_url}")
...
def check_case_with_langsearch(citation_text):
    ...
    try:
        ...
        if similarity >= 0.7:
            return {
                'is_real': True,
                'confidence': min(0.5 + similarity / 2, 0.9),  # Scale confidence based on similarity
...
def check_case_with_ai(citation_text, case_name=None):
    ...
    if USE_MULTI_SOURCE_VERIFIER:
        try:
            ...
            if result.get('found', False):
                print(f"Citation found and verified by enhanced verifier: {citation_text}")
                return result
            
            # If manual verification is suggested and enabled, add Descrybe.ai verification step
            if USE_DESCRYBE_MANUAL_VERIFICATION and result.get('manual_verification_suggested', False):
                # Create search URL for Descrybe.ai
                search_query = urllib.parse.quote(f"{citation_text} {case_name if case_name else ''}")
                descrybe_url = f"{DESCRYBE_URL}?q={search_query}"
                
                print(f"Suggesting manual verification with Descrybe.ai for citation: {citation_text}")
                return {
                    'found': False,
            ...
            print(f"Enhanced citation verifier couldn't confirm citation. Trying AI check as fallback.")
        except Exception as e:
    ...
    try:
        landmark_info = is_landmark_case(citation_text)
        if landmark_info:
            print(f"Found landmark case via AI check: {landmark_info['name']} ({citation_text})")
            return {
                'found': True,
                'url': f"https://supreme.justia.com/cases/federal/us/{citation_text.split()[0]}/{citation_text.split()[2]}/" if 'U.S.' in citation_text else None,
                'found_case_name': landmark_info['name'],
                'name_match': case_name and check_name_match(landmark_info['name'], case_name),
                'confidence': 0.95,
    ...
    try:
        ...
        first_summary = generate_langsearch_case_summary(citation_with_name, api_key, "detailed")
        second_summary = generate_langsearch_case_summary(citation_with_name, api_key, "brief")
        ...
        not_found_phrases = [
            "not a valid", "unable to find", "no information", "cannot find", 
            "do not have", "not a recognized", "not a known", "not familiar", 
            "no record", "not a real", "fictional", "i don't have", 
            "no results", "no matches", "couldn't locate", "could not locate",
            "i don't recognize", "i am not aware", "i cannot provide", "i don't have access",
            "not a case", "not an actual", "not a legitimate", "not a valid citation"
        ]
        ...
        if not case_name:
            ...
            first_line_1 = first_summary.split('\n')[0] if '\n' in first_summary else first_summary
            first_line_2 = second_summary.split('\n')[0] if '\n' in second_summary else second_summary
        ...
        if similarity >= 0.7:
            result = {
                ...
                'name_match': case_name and extracted_case_name and check_name_match(extracted_case_name, case_name),
                'confidence': min(0.5 + similarity / 2, 0.85),  # Scale confidence based on similarity
                'explanation': f"AI generated consistent summaries for this citation (similarity: {similarity:.2f}).",
                'source': 'AI Verification'
            }
            if urls:
                result['explanation'] += f" Found {len(urls)} reference URLs in summaries."
            return result
        elif similarity >= 0.4:
            result = {
                'found': True,
...
# Function to parse citation components
def parse_citation_components(citation_text):
    """
    Parse a citation into its component parts: volume, reporter, page, court, year.
    
    Args:
        citation_text (str): The citation to parse
        
    Returns:
        dict: Dictionary with citation components
    """
    components = {}
...
def generate_langsearch_case_summary(citation_text, api_key, summary_type):
    ...
    try:
        ...
        if word_count < 50:
            print(f"Summary too short ({word_count} words), requesting longer summary for {citation_text}")
...
def run_analysis(analysis_id, brief_text=None, file_path=None, api_key=None):
    ...
    try:
        ...
        analysis_results[analysis_id]['message'] = 'Pre-screening citations with ML classifier...'
            ...
            for i, citation in enumerate(citations):
                ...
                if isinstance(api_response, list):
                    ...
                    if case_name == 'Unknown case' or not case_name:
                        ...
                        brief_case_name = extract_case_name_from_text(brief_text, citation)
                        ...
                        # If we have a case name but no URL, also mark as unconfirmed
                            ...
                            if not found:
                                ...
                                if ml_confidence >= 0.7:
                                    confidence = 0.5  # Still lower than API-verified citations
                    ...
                    for i, citation in enumerate(citations):
                        ...
                        if found:
                            ...
                            if langsearch_result['is_real']:
                                ...
                                print(f"LangSearch couldn't verify citation, trying web search: {citation}")
                                
                                # Use our advanced case name extraction algorithm
                                case_name = extract_case_name_from_text(brief_text, citation)
                                if case_name:
                                    print(f"Found potential case name: {case_name}")
                                
                                # Try web search with the extracted case name
                                web_search_result = search_citation_on_web(citation, case_name)
                                ...
                                if web_search_result['found']:
                                    ...
                                    result_data = {
                                        ...
                                        'case_name': web_search_result['found_case_name'] or 'Unknown case'
                                    }
                                    
                                    # If case name was found and matches, increase confidence
                                    if web_search_result['name_match']:
                                        result_data['confidence'] = 0.9
                                        result_data['explanation'] += " (with matching case name)"
                                else:
                                    # Web search couldn't verify, try our third method: AI-based verification
                                    print(f"Web search couldn't verify citation, trying AI-based verification: {citation}")
                                    ...
                                    if ai_result['is_real']:
                                        ...
                                        if 'summaries' in ai_result:
                                            result_data['summaries'] = ai_result['summaries']
                                        
                                        # Add any URLs found in the summaries
                                        if 'urls' in ai_result and ai_result['urls']:
                                            result_data['reference_urls'] = ai_result['urls']
                                    else:
...
def serve_downloaded_briefs(filename):
    ...
    return send_from_directory(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'downloaded_briefs'), filename)

@app.route('/downloaded_briefs/exports/<path:filename>')
def serve_exports(filename):
    """Serve files from the exports directory."""
    return send_from_directory(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'downloaded_briefs', 'exports'), filename)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if file_path and not brief_text and USE_ENHANCED_VALIDATOR:
                try:
                    brief_text = extract_text_from_file(file_path)
                    print(f"Extracted {len(brief_text)} characters from file using Enhanced Validator")
                except Exception as e:
                    print(f"Error extracting text from file with Enhanced Validator: {e}")
                    traceback.print_exc()
            ...
            if brief_text and USE_ENHANCED_VALIDATOR:
                try:
                    ...
                    print(f"Validated {len(citations)} citations using Enhanced Validator")
...
def train_ml_classifier():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'message': 'Citation classifier trained successfully.'
        })
    except Exception as e:
        ...
        return jsonify({
            'success': False,
            'error': f'Error training classifier: {str(e)}'
        }), 500
...
def api_reprocess_citations():
    ...
    try:
        ...
        if 'citations' in data and isinstance(data['citations'], list):
            ...
            elif all(isinstance(c, dict) and 'citation_text' in c for c in data['citations']):
                citation_records = data['citations']
                citations_to_process = [record['citation_text'] for record in citation_records]
                print(f"Reprocessing {len(citations_to_process)} specific citations with source information")
        ...
        # Option 2: Load citations from database based on filter criteria
            ...
            try:
                ...
                citations_to_process = [record.get('citation_text') for record in citation_records if record.get('citation_text')]
                print(f"Loaded {len(citations_to_process)} unconfirmed citations from database")
            except ImportError:
                return jsonify({
        ...
        for i, citation_text in enumerate(citations_to_process):
            ...
            record = citation_records[i] if i < len(citation_records) else {'citation_text': citation_text}
            
            # Extract case name if possible
            case_name = record.get('case_name') or extract_case_name_from_text(citation_text, citation_text)
            ...
            if landmark_info:
                ...
                def simple_name_match(found_name, original_name):
                    if not original_name or not found_name:
                        return False
                    return found_name.lower() in original_name.lower() or original_name.lower() in found_name.lower()
                
                result = {
                    'found': True,
                    'url': f"https://supreme.justia.com/cases/federal/us/{citation_text.split()[0]}/{citation_text.split()[2]}/" if 'U.S.' in citation_text else None,
                    'found_case_name': landmark_info['name'],
                    'name_match': case_name and simple_name_match(landmark_info['name'], case_name),
                    'confidence': 0.95,
                    'explanation': f"Verified landmark case: {landmark_info['name']} ({landmark_info['year']}) - {landmark_info['significance']}",
                    'source': 'Landmark Cases Database'
                }
                ...
                if not result.get('found', False) and LANGSEARCH_API_KEY:
                    print(f"Web search didn't find citation, trying AI: {citation_text}")
        ...
        traceback.print_exc()
        return jsonify({
            'error': f'Error reprocessing citations: {str(e)}'
        }), 500
    ...
    try:
        ...
        if not filepath:
            return jsonify({'error': f'Failed to export citations in {format_type} format'}), 500
        ...
        traceback.print_exc()
        return jsonify({
            'error': f'Error exporting citations: {str(e)}'
        }), 500
...
def citation_tester():
    ...
    if request.method == 'POST':
        try:
            ...
            print(f"Processing citation test with count={citation_count}, confirmed={include_confirmed}, unconfirmed={include_unconfirmed}")
            ...
            if not test_citations:
                return render_template('citation_tester.html', error="No citations found matching your criteria. Please try different options.")
            ...
            for citation in test_citations:
                ...
                print(f"Testing citation: {citation_text} (Expected: {expected_result})")
                        ...
                        try:
                            ...
                            if result.get('found', False):
                                result = {
                                    ...
                                    'explanation': result.get('explanation', 'Verified by multi-source verification system'),
                                    'source': result.get('source', 'Multi-Source Verifier')
                                }
                                ...
                                result = {
                                    'found': False,
                                    'confidence': result.get('confidence', 0.7),
                                    'explanation': result.get('explanation', 'Citation format is valid but not found in legal databases'),
                                    'source': result.get('source', 'Multi-Source Verifier'),
                                    'format_analysis': result.get('format_analysis', {})
                                }
                        except Exception as e:
                ...
                actual_result = "confirmed" if result.get('found', False) else "unconfirmed"
            ...
            # Calculate accuracy
            accuracy = int((correctly_verified / len(test_citations)) * 100) if test_citations else 0
            ...
            traceback.print_exc()
            return render_template('citation_tester.html', error=f"An error occurred: {str(e)}")
...
def get_enhanced_citations():
    ...
    try:
        ...
        # Get enhanced citations from database
        filtered_citations.sort(key=lambda x: x.get('likelihood_of_being_real', 0.0), reverse=True)
...
def api_enhanced_validate_citation():
    ...
    Returns:
    - JSON with validation result including detailed citation components
    """
    try:
        data = request.get_json()
        ...
        # Check cache first
        cache_result = load_from_cache(citation)
        ...
        if cache_result:
            result.update({
                'verified': cache_result.get('found', False),
                'verified_by': cache_result.get('source'),
                'confidence': cache_result.get('confidence'),
                'url': cache_result.get('url')
            })
            return jsonify(result)
        
        # Use the multi-source verification system
        if MULTI_SOURCE_VERIFIER_AVAILABLE:
            verification_result = verify_citation_multi_source(citation, case_name=case_name)
        else:
            # Fall back to web search if multi-source verifier is not available
            verification_result = search_citation_on_web(citation, case_name=case_name)
        
        # Save to cache
        save_to_cache(citation, verification_result)
        
        # Update result with verification data
        result.update({
            'verified': verification_result.get('found', False),
            'verified_by': verification_result.get('source'),
...
def api_citation_context():
    ...
    Request body should be JSON with:
    - citation: Citation text to get context for
    
    Returns:
    - JSON with citation context and file link if available
    """
    try:
        data = request.get_json()
        citation = data.get('citation')
...
def api_suggest_citation_corrections():
    ...
    Request body should be JSON with:
    - citation: Citation text to suggest corrections for
    
    Returns:
    - JSON with correction suggestions
    """
    try:
        data = request.get_json()
        citation = data.get('citation')
        ...
        # Get correction suggestions
        suggestions = get_correction_suggestions(citation)
...
def api_verify_citation():
    ...
    try:
        ...
        # Use the multi-source verification system
        if MULTI_SOURCE_VERIFIER_AVAILABLE:
            result = verify_citation_multi_source(citation, case_name=case_name)
        else:
            # Fall back to web search if multi-source verifier is not available
            result = search_citation_on_web(citation, case_name=case_name)
        
        # Save to cache
        save_to_cache(citation, result)
...
# Helper function to check if a citation is a landmark case
def check_landmark_case(citation_text):
    """
...
@app.route('/confirmed-with-multitool')
def confirmed_with_multitool():
    ...
    """
    # Check if we're in the Vue.js environment
    if os.path.exists(os.path.join(os.path.dirname(__file__), 'static', 'vue')):
        return redirect('/casestrainer/#/multitool-confirmed')
    else:
        return redirect('/#tab-multitool-confirmed')
...
def confirmed_with_multitool_data():
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='multitool_confirmed_citations'")
        ...
        print(f"Found {len(citations)} citations in multitool_confirmed_citations table")
        conn.close()
...
@app.route('/unconfirmed_citations/data')
@app.route('/casestrainer/unconfirmed_citations/data')
def unconfirmed_citations_data():
    """
    API endpoint to provide data for the Unconfirmed Citations tab.
    """
    print("Unconfirmed Citations data endpoint called")
    try:
        conn = sqlite3.connect(DATABASE_FILE)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        # Check if the table exists
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='unconfirmed_citations'")
        if cursor.fetchone() is None:
            print("Table 'unconfirmed_citations' does not exist")
            
            # Try to load from the JSON file as a fallback
            try:
                json_path = os.path.join(os.path.dirname(__file__), 'downloaded_briefs', 'unconfirmed_citations_flat.json')
                if os.path.exists(json_path):
                    with open(json_path, 'r') as f:
                        citations = json.load(f)
                    print(f"Loaded {len(citations)} citations from JSON file")
                    return jsonify(citations)
                else:
                    print(f"JSON file not found at {json_path}")
                    return jsonify({'error': 'Data not found', 'citations': []})
            except Exception as json_error:
                print(f"Error loading from JSON: {json_error}")
                return jsonify({'error': f'JSON fallback failed: {str(json_error)}', 'citations': []})
        
        # Get all citations from the unconfirmed_citations table
        cursor.execute("""
        ...
        citations = [dict(row) for row in cursor.fetchall()]
        print(f"Found {len(citations)} citations in unconfirmed_citations table")
        conn.close()
        
        return jsonify(citations)
    except Exception as e:
        print(f"Error in unconfirmed_citations_data: {e}")
        return jsonify({'error': str(e), 'citations': []})
