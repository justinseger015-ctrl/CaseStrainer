...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("url_test_app.log"), logging.StreamHandler()],
)

# Create a logger for this module
logger = logging.getLogger("url_test_app")
...
logger.info("Starting URL Test Application")
...
def enhanced_analyze():
    """API endpoint to handle file uploads, text input, or URL input."""
    ...
    try:
        ...
        # Check if this is a URL input request
        # First, try to parse the request data as JSON
        try:
            ...
            if request.data and len(request.data) > 0:
                ...
                try:
                    ...
                    logger.info(f"JSON data parsed from request.data: {data}")

                    if data and "url" in data:
                        url = data["url"]
                        logger.info(f"URL detected in JSON data: {url}")

                        # Process the URL directly here
                        return direct_url_analyze(url)
                except json.JSONDecodeError as e:
                    logger.error(f"Error decoding JSON: {e}")
                    # Not JSON data, continue with other methods
        except Exception as e:
            logger.error(f"Error processing request data: {e}")
            # Continue with other input methods

        # If we reach here, it's not a URL input or there was an error
        return (
            jsonify({"status": "error", "message": "No file, text, or URL provided"}),
            400,
        )
...
def direct_url_analyze_endpoint():
    """API endpoint to directly analyze a URL for citations."""
    # Log request details for debugging
    ...
    try:
        ...
        logger.error(f"Error in direct_url_analyze_endpoint: {e}")
        return jsonify({"status": "error", "message": str(e)}), 500


def direct_url_analyze(url):
    """Function to analyze a URL for citations."""
    try:
        # Generate a unique ID for this analysis
        analysis_id = str(uuid.uuid4())
