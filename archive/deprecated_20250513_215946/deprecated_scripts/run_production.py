...
def main():
    ...
    parser.add_argument(
        "--unix-socket",
        help="Unix socket path (for Nginx, e.g., /tmp/casestrainer.sock)",
    )
    ...
    if not check_port_available(args.port):
        print(
            f"Warning: Port {args.port} is already in use. CaseStrainer may not start properly."
        )
    ...
    if nginx_status:
        ...
        print(
            "Warning: Nginx may not be running properly. External access through https://wolf.law.uw.edu/casestrainer/ might not work."
        )
        print(
            f"However, CaseStrainer will still be accessible locally at http://127.0.0.1:{args.port}"
        )
    ...
    try:
        ...
        app_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "app_final.py"
        )
