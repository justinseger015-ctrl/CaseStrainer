...
class WorkingFallbackVerifier:
    ...
    async def _verify_with_justia(self, citation_text: str, citation_info: Dict[str, Any],
        ...
        # Type guard to ensure citation_info is not None
        if citation_info is None:
            logger.warning(f"citation_info is None for {citation_text}")
            return None
        self._init_session()
    ...
    async def _verify_with_vlex(self, citation_text: str, citation_info: Dict[str, Any],
        ...
        NOTE: This function is deprecated in this file. Use EnhancedFallbackVerifier for proper vLex support.
        vLex remains in the pipeline and is fully implemented in the enhanced verifier.
        """
        logger.info(f"vLex verification redirected to EnhancedFallbackVerifier for {citation_text}")
        # Redirect to enhanced verifier which has proper vLex implementation
        try:
            from src.enhanced_fallback_verifier import EnhancedFallbackVerifier
            verifier = EnhancedFallbackVerifier()
            result = await verifier._verify_with_vlex(citation_text, citation_info, extracted_case_name, extracted_date)
            return result
        except Exception as e:
            logger.warning(f"Enhanced vLex verification failed for {citation_text}: {e}")
            return None
    ...
    def _extract_case_name_from_vlex_link(self, link_text: str) -> Optional[str]:
        ...
        NOTE: This function is deprecated in this file. Use EnhancedFallbackVerifier for proper vLex support.
        """
        logger.info("vLex case name extraction redirected to EnhancedFallbackVerifier")
        # Redirect to enhanced verifier which has proper vLex implementation
        try:
            from src.enhanced_fallback_verifier import EnhancedFallbackVerifier
            verifier = EnhancedFallbackVerifier()
            # Note: This is a sync method, so we'll need to handle it appropriately
            return None  # Enhanced verifier handles this asynchronously
        except Exception as e:
            logger.warning(f"Enhanced vLex case name extraction failed: {e}")
            return None
