...
class ComprehensiveWebExtractor:
    ...
    def _is_valid_case_name(self, case_name: str) -> bool:
        ...
        # Must contain "v." or "vs." or "versus"
        if not re.search(r'\bv\.?\b|\bvs\.?\b|\bversus\b', case_name, re.IGNORECASE):
            return False
        
        # Must have at least two words before "v."
        ...
        # Both plaintiff and defendant should have at least one word
    ...
    def _calculate_case_name_confidence(self, case_name: str, text: str) -> float:
        ...
        # Check if case name appears multiple times
        occurrences = text.lower().count(case_name.lower())
        if occurrences > 1:
            confidence += 0.2
        
        # Check for citation nearby
        if self._has_citation_nearby(case_name, text):
            confidence += 0.3
        
        # Check case name length (longer names are more specific)
        if len(case_name) > 20:
            confidence += 0.1
    ...
    def _extract_case_name(self, soup, citation: str) -> str:
        ...
        # Look for case name in various elements
        case_name_selectors = [
            '.case-name',
            '.title',
            'h1',
        ...
        # Try to extract from context around citation
