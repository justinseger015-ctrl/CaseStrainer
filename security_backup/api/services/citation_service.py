        ...
        try:
            if input_data.get('type') == 'text':
                ...
                if len(text) < 300 and len(citations) <= 3:
                    ...
                    # For Washington citations, use fast verification that prioritizes speed
                    ...
                    # BUT only when clustering is actually needed (not for very short text)
                    if any('Wn.' in str(c) for c in citation_results) and len(text) >= 300:
                        ...
                        # BUT use fast verification to get canonical data (names, years, URLs) quickly
                        enable_verification = True
                        logger.info(f"[CitationService] Enabling fast verification for Washington citations")
                    elif any('Wn.' in str(c) for c in citation_results) and len(text) < 300:
                        logger.info(f"[CitationService] Washington citations detected but text too short - using fast path")
