    ...
    try {
        ...
        if (-not $SkipVueBuild -and -not $QuickStart) {
            $vueJob = Start-Job -ScriptBlock {
                $vueDir = Join-Path $using:ScriptRootPath "casestrainer-vue-new"
                $distDir = Join-Path $vueDir "dist"
                return (-not (Test-Path $distDir) -or $using:ForceRebuildFlag)
            } -ArgumentList $PSScriptRoot, $ForceRebuild.IsPresent
        }
...
function Show-Menu {
    ...
    Write-Host ""
    Write-Host " 4. ⚙️  Advanced Diagnostics" -ForegroundColor Blue
    Write-Host " 5. 🗃️  Cache Management" -ForegroundColor Yellow
    Write-Host " 6. ⏹️  Stop All Services" -ForegroundColor Red
