...
def get_brief_links_from_division(division_url, max_briefs=100):
    ...
    try:
        ...
        for link in soup.find_all('a'):
            href = link.get('href')
            text = link.text.strip().lower()
            if href and href.endswith('.pdf') and ('brief' in text or 'petition' in text or 'motion' in text):
                full_url = urljoin(WA_COURTS_BASE_URL, href)
...
def main():
    ...
    for division_name, division_url in division_links:
        ...
        for i, brief_url in enumerate(new_briefs):
            print(f"Processing brief {i+1}/{len(new_briefs)} from {division_name}: {brief_url}")
    ...
    print(f"Found {len(all_unverified_citations)} unverified citations in {total_briefs_processed} briefs")
