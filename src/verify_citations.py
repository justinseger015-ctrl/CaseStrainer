...
def test_against_landmark_cases(citations_to_test=None):
    ...
    if citations_to_test:
        test_citations = [c for c in flat_citations if c['citation_text'] in citations_to_test]
    else:
        test_citations = flat_citations
    
    print(f"Testing {len(test_citations)} unconfirmed citations against landmark cases database...")
    ...
    for citation in test_citations:
        ...
        if landmark_info:
            ...
            print(f"UNCONFIRMED: Citation not found in landmark cases database: {citation_text}")
...
def main():
    ...
    print(f"Newly confirmed citations: {results['summary']['newly_confirmed_count']} ({results['summary']['percentage_confirmed']}%)")
    print(f"Still unconfirmed citations: {results['summary']['still_unconfirmed_count']}")
    ...
    if results['newly_confirmed']:
        ...
        for i, citation in enumerate(results['newly_confirmed'], 1):
            ...
            print(f"{i}. {citation['citation_text']} ({citation.get('case_name', 'N/A')})")
