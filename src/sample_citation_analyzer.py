...
def check_citation_in_courtlistener(citation_text):
    ...
    try:
        ...
        # Make the request
        response = requests.get('https://www.courtlistener.com/api/rest/v3/search/', headers=headers, params=params)
...
def process_sample_text():
    ...
    real_citations = extract_citations_with_context(SAMPLE_TEXT_WITH_CITATIONS, "Sample Text (Real Citations)")
    
    # Extract citations from the made-up citations sample text
    made_up_citations = extract_citations_with_context(SAMPLE_TEXT_WITH_MADE_UP_CITATIONS, "Sample Text (Made-up Citations)")
