...
def save_processed_briefs(processed_briefs):
    ...
    try:
        ...
        logger.info(f"Saved {len(processed_briefs)} processed briefs to {PROCESSED_BRIEFS_CACHE}")
    except Exception as e:
...
def verify_citation(citation, multi_source_verifier):
    ...
    try:
        ...
        logger.info(f"Verification result for {citation['citation_text']}: found={result['found']}, source={result.get('source', 'None')}")
        return result
...
def update_citation_json_files(multitool_citations, unconfirmed_citations):
    ...
    for citation in unconfirmed_citations:
        citation_data["still_unconfirmed"].append({
            ...
            "explanation": citation.get('explanation', 'Citation could not be verified'),
    ...
    database_file = os.path.join(os.path.dirname(__file__), 'database_verification_results.json')
    database_data = []
    ...
    for citation in multitool_citations:
        database_data.append({
            ...
            "explanation": citation.get('explanation', 'Citation verified with alternative source')
        })
...
def process_brief(brief_info, processed_briefs, multi_source_verifier):
    ...
    try:
        ...
        logger.info(f"Processed brief {brief_path}: {len(multitool_citations)} multitool citations, {len(unconfirmed_citations)} unconfirmed citations")
...
def main():
    ...
    if not briefs_to_process:
        ...
        logger.info(f"No briefs metadata found, scanning {BRIEFS_DIR} directory for PDFs")
    ...
    new_briefs = [brief for brief in briefs_to_process if brief['path'] not in processed_briefs]
    ...
    for i, brief in enumerate(new_briefs):
        ...
        multitool_citations, unconfirmed_citations = process_brief(brief, processed_briefs, multi_source_verifier)
    ...
    logger.info(f"Found {len(all_multitool_citations)} citations verified with multitool")
