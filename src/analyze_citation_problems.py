...
def generate_report(type_stats, flat_citations):
    ...
    with open(report_file, "w") as f:
        ...
        f.write(
            f"Valid Formats: {valid_format_count} "
            f"({valid_format_count/len(flat_citations)*100:.1f}%)\n"
        )
        f.write(
            f"Valid Volumes: {valid_volume_count} "
            f"({valid_volume_count/len(flat_citations)*100:.1f}%)\n\n"
        )
        ...
        for format_type, stats in sorted_types:
            ...
            f.write(
                f"Count: {stats['count']} "
                f"({stats['count']/len(flat_citations)*100:.1f}%)\n"
            )
            f.write(
                f"Valid Formats: {stats['valid_format_count']} "
                f"({stats['valid_format_percent']:.1f}%)\n"
            )
            f.write(
                f"Valid Volumes: {stats['valid_volume_count']} "
                f"({stats['valid_volume_percent']:.1f}%)\n"
            )
    ...
    try:
        ...
        if type_stats:
            ...
            for format_type, stats in sorted(
                ...
                print(
                    f"  {format_type}: {stats['count']} citations, "
                    f"{stats['valid_format_percent']:.1f}% valid format, "
                    f"{stats['valid_volume_percent']:.1f}% valid volume"
                )
