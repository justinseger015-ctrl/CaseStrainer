...
class MultiSourceVerifier:
    ...
    def verify_citation(self, citation_text, case_name=None, sources=None):
        ...
        search_query = urllib.parse.quote(f"{normalized_citation} {case_name if case_name else ''}")
        ...
        for format_type, pattern in CITATION_PATTERNS.items():
            ...
            if match:
                ...
                if format_type == "us_reports":
                    ...
                    # Check volume range
                    if volume < VALID_VOLUME_RANGES["us_reports"][0] or volume > VALID_VOLUME_RANGES["us_reports"][1]:
                        return {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("count", 0) > 0:
                    ...
                    name_match = check_name_match(case_name, result_case_name) if case_name else True
    ...
    def check_google_scholar(self, citation_text, case_name=None):
        ...
        try:
            response = self.session.get(
                VERIFICATION_SOURCES["google_scholar"]["base_url"],
                params=params
            )
            ...
            if response.status_code == 200:
                ...
                if results:
                    ...
                    # Check if the case name matches
                    name_match = check_name_match(case_name, title) if case_name else True
    ...
    def check_justia(self, citation_text, case_name=None):
        ...
        try:
            response = self.session.get(
                VERIFICATION_SOURCES["justia"]["base_url"],
                params=params
            )
...
if __name__ == "__main__":
    ...
    for citation in test_citations:
        ...
        if result.get("manual_verification_suggested", False):
            print(f"Manual verification suggested: {result.get('manual_verification_source')}")
            print(f"Manual verification URL: {result.get('manual_verification_url')}")
            print(f"Manual verification message: {result.get('manual_verification_message')}")
