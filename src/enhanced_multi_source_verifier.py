...
class EnhancedMultiSourceVerifier:
    ...
    def __init__(self):
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                ...
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "fixed_multi_source_verifier.py")
            )
    ...
    def _check_cache(self, citation):
        ...
        cache_file = os.path.join(self.cache_dir, f"{self._normalize_citation(citation).replace(' ', '_')}.json")
    ...
    def _verify_with_courtlistener(self, citation):
        ...
        if not self.courtlistener_api_key:
            return {"verified": False, "source": "CourtListener", "error": "No API key provided"}
        ...
        try:
            ...
            if data.get("count", 0) > 0:
                for result in data.get("results", []):
                    ...
                    if (components["volume"] in citation_string and
                        components["reporter"] in citation_string and
                        components["page"] in citation_string):
                        return {
                            "verified": True,
                            "source": "CourtListener",
            ...
            logger.error(f"Error verifying citation '{citation}' with CourtListener: {e}")
            return {
                "verified": False,
                "source": "CourtListener",
                "error": str(e)
    ...
    def _verify_with_langsearch(self, citation):
        ...
        if not self.langsearch_api_key:
            return {"verified": False, "source": "LangSearch", "error": "No API key provided"}
        ...
        try:
            ...
            if results.get("results", []):
                for result in results.get("results", []):
                    ...
                    if (components["volume"] in content and
                        components["reporter"] in content and
                        components["page"] in content):
                        return {
                            "verified": True,
                            "source": "LangSearch",
                            "content": content,
    ...
    def _verify_with_database(self, citation):
        ...
        try:
            ...
            cursor.execute("SELECT * FROM citations WHERE citation_text = ? AND found = 1", (citation,))
            ...
            cursor.execute("SELECT * FROM citations WHERE citation_text = ? AND found = 1", (normalized_citation,))
    ...
    def _verify_with_landmark_cases(self, citation):
        ...
        try:
            # Load landmark cases
            landmark_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "landmark_cases.json")
            ...
            # Check for normalized match
            for case in landmark_cases:
                if normalized_citation == self._normalize_citation(case.get("citation", "")):
                    return {
                        "verified": True,
                        "source": "Landmark Cases",
                        "match_type": "normalized",
            ...
            if components["volume"] and components["reporter"] and components["page"]:
                for case in landmark_cases:
                    case_components = self._extract_citation_components(case.get("citation", ""))
                    if (components["volume"] == case_components["volume"] and
                        components["reporter"] == case_components["reporter"] and
                        components["page"] == case_components["page"]):
                        return {
                            "verified": True,
                            "source": "Landmark Cases",
                            "match_type": "component",
            ...
            logger.error(f"Error verifying citation '{citation}' with landmark cases: {e}")
            return {
                "verified": False,
                "source": "Landmark Cases",
                "error": str(e)
    ...
    def _verify_with_fuzzy_matching(self, citation):
        ...
        try:
            ...
            logger.error(f"Error verifying citation '{citation}' with fuzzy matching: {e}")
            return {
                "verified": False,
                "source": "Fuzzy Matching",
                "error": str(e)
    ...
    def _verify_with_original_verifier(self, citation):
        ...
        try:
            ...
            logger.error(f"Error verifying citation '{citation}' with original verifier: {e}")
            return {
                "verified": False,
                "source": "Original Verifier",
                "error": str(e)
