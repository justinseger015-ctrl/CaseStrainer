...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("wa_briefs_extraction.log"),
...
try:
    ...
    logger.info("Successfully imported MultiSourceVerifier from fixed_multi_source_verifier")
except ImportError:
    try:
        from multi_source_verifier import MultiSourceVerifier
        logger.info("Successfully imported MultiSourceVerifier from multi_source_verifier")
    except ImportError:
        logger.error("Error importing MultiSourceVerifier, trying to import from app_final")
        try:
...
try:
    with open('config.json', 'r') as f:
        ...
        logger.info(f"Loaded CourtListener API key from config.json: {api_keys['courtlistener'][:5]}...")
...
def get_court_of_appeals_links():
    ...
    try:
        ...
        for link in soup.find_all('a'):
            ...
            if href and ('division-i' in href.lower() or 'division-ii' in href.lower() or 'division-iii' in href.lower()):
...
def get_brief_links_from_division(division_url, max_briefs=MAX_BRIEFS_PER_DIVISION):
    ...
    try:
        ...
        for link in soup.find_all('a'):
            ...
            if href and href.lower().endswith('.pdf') and ('brief' in href.lower() or 'appellant' in href.lower() or 'respondent' in href.lower()):
                full_url = urljoin(WA_COURTS_BASE_URL, href)
...
def process_brief(brief_url, processed_briefs):
    ...
    try:
        ...
        for citation in citations:
            ...
            try:
                ...
                if not result['found']:
                    ...
                    context_match = re.search(r'(.{0,100}' + re.escape(citation) + r'.{0,100})', text)
        ...
        logger.info(f"Found {len(unverified_citations)} unverified citations in {brief_path}")
...
def save_unverified_citations(unverified_citations):
    ...
    try:
        ...
        citation_file = os.path.join(os.path.dirname(__file__), 'citation_verification_results.json')
        citation_data = {
            "newly_confirmed": [],
            "still_unconfirmed": []
        }
        ...
        for citation_info in unverified_citations:
            citation_data["still_unconfirmed"].append({
                ...
                "explanation": citation_info['verification_result'].get('explanation', 'Citation could not be verified'),
                "confidence": citation_info['verification_result'].get('confidence', 0.1),
        ...
        logger.info(f"Saved {len(unverified_citations)} unverified citations to {citation_file} and {OUTPUT_FILE}")
...
def main():
    ...
    logger.info("Starting extraction of unverified citations from Washington Court briefs")
    ...
    for division_name, division_url in division_links:
        ...
        brief_links = get_brief_links_from_division(division_url, max_briefs=MAX_BRIEFS_PER_DIVISION)
        ...
        for i, brief_url in enumerate(new_briefs):
            logger.info(f"Processing brief {i+1}/{len(new_briefs)} from {division_name}: {brief_url}")
            ...
            if total_briefs_processed >= MAX_TOTAL_BRIEFS:
                logger.info(f"Reached limit of {MAX_TOTAL_BRIEFS} briefs processed. Stopping.")
                break
    ...
    logger.info(f"Found {len(all_unverified_citations)} unverified citations in {total_briefs_processed} briefs")
