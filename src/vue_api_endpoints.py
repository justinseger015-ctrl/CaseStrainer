...
def import_enhanced_verifier():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            ...
            os.path.join(os.path.dirname(__file__), "enhanced_multi_source_verifier.py")
        )
        ...
        try:
            ...
            for citation in citations:
                ...
                citation_data = {
                    ...
                    'context': citation['context'] if 'context' in citation.keys() else '',
                    'file_link': citation['file_link'] if 'file_link' in citation.keys() else '',
                    'volume': citation['volume'] if 'volume' in citation.keys() else '',
                    'reporter': citation['reporter'] if 'reporter' in citation.keys() else '',
...
def reprocess_citation():
    ...
    try:
        ...
        cursor.execute('SELECT id, citation_text, found FROM citations WHERE citation_text = ?', (citation,))
        ...
        if enhanced_verifier:
            ...
            explanation = result.get('verified_by', 'Unknown') if verified else result.get('error', 'Unknown error')
        ...
        return jsonify({
            'success': False,
            'message': f'Error reprocessing citation: {str(e)}'
        }), 500
...
def enhanced_validate_citation():
    ...
    try:
        ...
        if enhanced_verifier:
            ...
            cursor.execute('SELECT found FROM citations WHERE citation_text = ?', (citation,))
            citation_record = cursor.fetchone()
...
def get_citation_context():
    ...
    try:
        ...
        cursor.execute('SELECT context, file_link FROM citations WHERE citation_text = ?', (citation,))
