...
from courtlistener_integration import batch_citation_validation


from config import DATABASE_FILE  # Use centralized config
...
def confirmed_with_multitool_data():
    ...
    Returns citations that were confirmed with the multi-source tool
    but not with
    CourtListener.
    """
    try:
        ...
        try:
            ...
            # Get citations confirmed with multitool
            # but not with CourtListener
            ...
            for citation in citations:
                ...
                citation_data = {
                    ...
                    "case_name": (
                        citation["case_name"] if citation["case_name"] else ""
                    ),
                    ...
                    "volume": (
                        citation["volume"] if "volume" in citation.keys() else ""
                    ),
                    "reporter": (
                    ...
                    "metadata": (
                        citation["metadata"]
                        if "metadata" in citation.keys()
                        and citation["metadata"] is not None
                        else {}
                    ),
                    "backdrop": (
                        citation["backdrop"]
                        if "backdrop" in citation.keys()
                        and citation["backdrop"] is not None
                        else ""
                    ),
                }
                if "contexts" in citation_data and isinstance(
                    citation_data["contexts"], list
                ):
                    for context in citation_data["contexts"]:
                        if isinstance(context.get("text"), str) and context[
                            "text"
            ...
            result = [
                {
                    ...
                    "url": (
                        ...
                        "scholar_case?case=12120372216939101759"
                    ),
                    "metadata": {},
                    "backdrop": "",
                    "context": (
                        ...
                        "(347 U.S. 483) "
                        "established that separate educational facilities "
                ...
                },
                {
                    ...
                    "backdrop": "",
                    "context": (
                        "The Court's decision in Roe v. Wade (410 U.S. 113) "
                ...
                },
                {
                    ...
                    "backdrop": "",
                    "context": (
                        "Marbury v. Madison (5 U.S. 137) established the "
                ...
                if "contexts" in citation_data and isinstance(
                    ...
                    for context in citation_data["contexts"]:
                        if isinstance(context.get("text"), str) and context[
                            ...
                            context["text"] = ""
                # Guarantee 'contexts' is always a list
                if "contexts" not in citation_data or not isinstance(
                    citation_data["contexts"], list
                ):
                    citation_data["contexts"] = []
                result.append(citation_data)
        except Exception as db_error:
            # If there's a database error (like missing table), use sample data
            print(f"Database error: {str(db_error)}")
            ...
            result = [
                {
                    ...
                    "confidence": 0.15,
                    "explanation": (
                        "Citation not found in any legal database. "
                        "The U.S. Reports volume 999 does not exist."
                    ),
                    "source_document": "sample_brief_1.pdf",
                    "metadata": {},
                    "backdrop": "",
                    "context": (
                        "According to Fictional v. NonExistent (999 U.S. 123), "
                        "the court held that..."
                    ),
                    "date_added": datetime.now().isoformat(),
                },
                {
                    ...
                    "confidence": 0.22,
                    "explanation": (
                        "Citation format is valid but no matching case found. "
                        "The F.3d volume 531 does not contain a page 9999."
                    ),
                    "source_document": "sample_brief_2.pdf",
                    "metadata": {},
                    "backdrop": "",
                    "context": (
                        "In Smith v. Imaginary Corp (531 F.3d 9999), "
                ...
                },
                {
                    ...
                    "confidence": 0.30,
                    "explanation": (
                        "No case with this Westlaw citation could be found. "
                        "The citation format is valid but the identifier "
                        "does not exist."
                    ),
...
def reprocess_citations():
    """
    API endpoint to reprocess multiple unconfirmed citations to check
    if they can now be confirmed.
    """
    try:
                ...
                "results": [
                    {
                        ...
                        "confidence": 0.25,
                        "explanation": (
                            "Citation still could not be verified after reprocessing."
                        ),
                    }
                    for citation in citations
                ],
...
def reprocess_citation():
    """
    API endpoint to reprocess a single unconfirmed citation to check
    if it can now be confirmed.
    """
    try:
...
@vue_api.route("/api/enhanced-validate-citation", methods=["POST"])
@vue_api.route("/casestrainer/api/enhanced-validate-citation", methods=["POST"])
def enhanced_validate_citation():
    """
    API endpoint to validate a citation using the enhanced
    ...
    try:
        ...
        if enhanced_verifier:
            result = enhanced_verifier.verify_citation(citation)
            # Ensure metadata and backdrop fields are present in the
            # response if it's a citation dict
            if isinstance(result, dict):
                if "metadata" not in result or result["metadata"] is None:
                    result["metadata"] = {}
                if "backdrop" not in result or result["backdrop"] is None:
                    result["backdrop"] = ""
        ...
        return (
            jsonify(
                {
                    "verified": False,
                    "error": f"Error validating citation: {str(e)}",
                    "metadata": {},
                    "backdrop": "",
                }
            ),
...
# API endpoint for batch citation validation
@vue_api.route("/batch_validate_citations", methods=["POST"])
def batch_validate_citations_api():
    """
    API endpoint to validate a list of citations using CourtListener API
    with fallback.
    Expects JSON: {"citations": [list of citation strings]}
    Returns: {"results": [ {citation, exists, method, error} ]}
    """
    data = request.get_json(force=True)
    citations = data.get("citations", [])
    if not isinstance(citations, list):
        return jsonify({"error": "'citations' must be a list"}), 400
    results = batch_citation_validation(citations)
