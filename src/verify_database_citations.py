...
try:
    ...
    print("Error: Could not import MultiSourceVerifier. Make sure you're running this script from the CaseStrainer directory.")
    sys.exit(1)
...
def get_database_citations():
    ...
    cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='unconfirmed_database_citations'")
    if not cursor.fetchone():
        ...
        ''')
        conn.commit()
        print("Table created, but it's empty. Please add citations to the Unconfirmed Citations Database tab.")
        return []
    ...
    print(f"Retrieved {len(citations)} citations from the Unconfirmed Citations Database tab")
    return citations
...
def update_citation_verification(citation_id, verification_result):
    ...
    if 'verification_status' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_database_citations ADD COLUMN verification_status TEXT")
    
    if 'verification_confidence' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_database_citations ADD COLUMN verification_confidence REAL")
    
    if 'verification_source' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_database_citations ADD COLUMN verification_source TEXT")
    
    if 'verification_tag' not in columns:
        cursor.execute("ALTER TABLE unconfirmed_database_citations ADD COLUMN verification_tag TEXT")
...
def main():
    ...
    print("Starting verification of citations from the Unconfirmed Citations Database tab...")
    ...
    # Analyze results
    verified_count = sum(1 for r in verification_results if r.get('verification_result', {}).get('found', False))
    multi_source_only = sum(1 for r in verification_results 
    ...
    print(f"Multi-source only verifications: {multi_source_only} ({multi_source_only/verified_count:.2%} of verified)" if verified_count > 0 else "Multi-source only verifications: 0 (0.00%)")
