...
def find_similar_citations(citation_text, api_key=None):
                ...
                if response.status_code == 200:
                    ...
                    for result in results:
                        ...
                        if citation:
                            ...
                            similarity = fuzz.ratio(normalize_citation(citation_text), normalize_citation(citation))
    ...
    confirmed_citations = [c for c in unconfirmed_citations if c.get('confidence', 0) >= 0.7]
    
    for citation in confirmed_citations:
        confirmed_text = citation.get('citation_text', '')
        if confirmed_text:
            # Calculate similarity score
            similarity = fuzz.ratio(normalize_citation(citation_text), normalize_citation(confirmed_text))
            ...
            if ('wl_year' in components and 'wl_year' in confirmed_components and
                components['wl_year'] == confirmed_components['wl_year']):
                component_match = True
...
def suggest_corrections_for_all():
    ...
    low_confidence_citations = [c for c in unconfirmed_citations if c.get('confidence', 0) < 0.7]
    
    print(f"Generating correction suggestions for {len(low_confidence_citations)} unconfirmed citations")
