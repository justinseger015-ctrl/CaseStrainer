...
if not os.path.exists(BRIEFS_DIR):
    try:
        ...
        try:
            import subprocess
            subprocess.run(["powershell", "-Command", f"New-Item -ItemType Directory -Path '{BRIEFS_DIR}' -Force"], check=True)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                import subprocess
                subprocess.run(["powershell", "-Command", f"Copy-Item -Path '{temp_path}' -Destination '{file_path}' -Force"], check=True)
                os.remove(temp_path)
...
def main():
    ...
    logger.info(f"Finished downloading briefs. Downloaded {downloaded_count} new briefs.")
