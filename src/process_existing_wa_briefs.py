...
def extract_citations_with_context(text):
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, text):
            ...
            existing = next((item for item in citation_data if item['citation'] == citation), None)
            if not existing:
                ...
                existing['positions'].append({'start': match.start(), 'end': match.end()})
    ...
    try:
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                ...
                os.path.join(os.path.dirname(os.path.abspath(__file__)), "enhanced_multi_source_verifier.py")
            )
...
def add_to_database(citation_data, source, verification_result, file_link=None):
    ...
    try:
        ...
        cursor.execute("SELECT id FROM citations WHERE citation_text = ?", (citation_data['citation'],))
        ...
        if existing:
            ...
            cursor.execute(
                ...
                (
                    ...
                    verification_result.get("verified_by", "") if verification_result.get("verified", False) else verification_result.get("error", ""),
            ...
            logger.info(f"Updated existing citation in database: {citation_data['citation']}")
        else:
...
def process_brief(brief_path):
    ...
    try:
        ...
        for citation_item in citation_data:
            ...
            add_to_database(citation_item, f"WA Brief: {file_name}", verification_result, file_link)
...
def save_unverified_citations(unverified_citations):
    ...
    try:
        ...
        logger.info(f"Saved {len(unverified_citations)} unverified citations to {CITATIONS_FILE}")
...
def main():
    ...
    logger.info("Starting Washington briefs processing with enhanced citation validation")
    ...
    try:
        ...
        required_columns = ['context', 'file_link', 'volume', 'reporter', 'page', 'court', 'year']
    ...
    logger.info("Finished processing Washington briefs with enhanced citation validation")
    ...
    print("3. Test the enhanced citation validation with some of the extracted citations")
