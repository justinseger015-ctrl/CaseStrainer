...
def extract_citations_from_cases():
    ...
    try:
        ...
        for case in cases:
            ...
            case_name = case.get('case_name', '')
            citation_match = re.search(r'(\d+\s+Wash\.\s+(?:2d|App\.)?\s+\d+)', case_name)
            if citation_match:
                citations.append({
            ...
            if url:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        citation_match = re.search(r'(\d+\s+Wash\.\s+(?:2d|App\.)?\s+\d+)', response.text)
                        if citation_match:
                            citations.append({
                                'citation': citation_match.group(1),
                                'case_name': case.get('case_name'),
        ...
        print(f"Extracted {len(citations)} citations and saved to washington_citations.json")
        return citations
