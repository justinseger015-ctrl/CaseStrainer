...
LEGAL_TOPICS = [
    "Constitutional Law", "Criminal Law", "Civil Procedure", "Contracts", "Torts",
    "Property Law", "Administrative Law", "Evidence", "Family Law", "Environmental Law",
    "Intellectual Property", "Tax Law", "Corporate Law", "Securities Regulation",
    "Antitrust Law", "Bankruptcy", "Employment Law", "Immigration Law", "International Law",
    "Health Law", "Education Law", "Elder Law", "Disability Law", "Consumer Protection",
    "Civil Rights", "First Amendment", "Fourth Amendment", "Due Process", "Equal Protection"
]
...
def save_unconfirmed_citations(citations, filename='downloaded_briefs/enhanced_unconfirmed_citations.json'):
    ...
    for format_type, pattern in CITATION_PATTERNS.items():
        ...
        if match:
            ...
            if format_type == "us_reports":
                ...
                if series_num > 3:
                    return (format_type, False, False, {"error": "Invalid series number (too high)"})
            ...
            return (format_type, True, True, {"note": "Format recognized but detailed validation not implemented"})
...
def categorize_citation(citation):
    ...
    format_type, is_valid_format, is_valid_volume, details = analyze_citation_format(citation_text)
    ...
    if is_valid_format and is_valid_volume:
        ...
        likelihood = random.uniform(0.2, 0.4)  # Lower likelihood for valid format but invalid volume
    else:
...
def scan_and_categorize_citations():
    ...
    print(f"Valid formats: {valid_format_count} ({valid_format_count/total_citations*100:.1f}%)")
    print(f"Valid volumes: {valid_volume_count} ({valid_volume_count/total_citations*100:.1f}%)")
    print("\nFormat Types:")
    for format_type, count in sorted(format_types.items(), key=lambda x: x[1], reverse=True):
...
def create_test_api_request(enhanced_citations, max_citations=10):
    ...
    flat_citations.sort(key=lambda x: x.get("likelihood_of_being_real", 0), reverse=True)
    ...
    try:
        ...
        ps_command += "$body = Get-Content -Raw -Path 'enhanced_test_api_request.json'; "
