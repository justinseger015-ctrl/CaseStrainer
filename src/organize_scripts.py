...
for py_file in BASE_DIR.glob('*.py'):
    if py_file.name not in essential_scripts and py_file.name not in [os.path.basename(s) for s in deprecated_scripts]:
...
if other_scripts:
    ...
    print("Enter the numbers separated by commas, or 'all' to move all, or 'none' to keep all:")
    choice = input("> ")
...
Last updated: {}
""".format(
    '\n'.join(f"- **{script}**" for script in os.listdir(deprecated_dir) if script.endswith('.py')),
